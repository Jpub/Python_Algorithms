...
for i in range(PEOPLE):
    first_names[i] = input("영문 성을 입력하여라(" +      \
                            str(i + 1) + "): ")
    last_names[i] = input("영문 이름을 입력하여라(" +        \
                           str(i + 1) + "): ")
    ages[i] = get_age()

index_of_max = find_max(ages)

print("나이가 가장 많은 사람:", first_names[index_of_max],        \
                               last_names[index_of_max])
